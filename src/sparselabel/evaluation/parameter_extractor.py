...
class ParameterExtractorBBMRI:
    ...
    def net_flow(self, mask, throughplane_flow):
        if np.sum(np.astype(mask==self._dataset_config.class_value_by_label(DatasetInfo.LUMEN), int))==0:
            return None
        return np.abs(np.sum(np.astype(mask==self._dataset_config.class_value_by_label(DatasetInfo.LUMEN), int) *throughplane_flow)*(self._mpr_resolution[0]*self._mpr_resolution[1]))
...
class Aorta:
    ...
    def evaluate(self, truth: CrossSection, case):
        ...
        relevant_mask = self._cut_mask_between_lumen(mpr)
        lumen_radius = self.lumen_radius_manual(relevant_mask)
        lumen_radius_manual = self.lumen_radius_manual(truth.lumen_points, truth._outer_wall_contour_points)
    ...
    def _mpr(self, cross_section, case):
        ...
        x_vals = (np.arange(self._mpr_shape[0]) - self._mpr_shape[0] / 2) * self._mpr_resolution[0]
        y_vals = (np.arange(self._mpr_shape[1]) - self._mpr_shape[1] / 2) * self._mpr_resolution[1]
        x_grid, y_grid = np.meshgrid(x_vals, y_vals)
        grid_world_coords = (cross_section.plane_center +
                             x_grid[..., None] * x_axis +
                             y_grid[..., None] * y_axis)
